...
// Генерация случайного IV/nonce для CTR
...
// Шифрование данных алгоритмом Магма в режиме CTR
std::vector<unsigned char> encrypt_magma_ctr(const std::vector<unsigned char>& plaintext,
    ...
    // Устанавливаем алгоритм Магма (GOST 28147-89) в режиме CTR
    EVP_CIPHER* cipher = EVP_CIPHER_fetch(nullptr, "magma-ctr", nullptr);
    if (!cipher) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 4. Генерация IV для CTR
        std::vector<unsigned char> iv = generate_iv();
        
        // 5. Шифрование данных в режиме CTR
        std::cout << "Encrypting with Magma (GOST 28147-89) in CTR mode...\n";
        size_t block_count = 0;
        auto encrypt_start = std::chrono::high_resolution_clock::now();
        std::vector<unsigned char> ciphertext = encrypt_magma_ctr(file_data, alice_secret, iv, block_count);
        ...
        // 8. Сохранение зашифрованных данных и IV
